...
public class DragonEntity extends HorseBase implements CustomEntity, EntityInteractable {
    ...
    private static final int DISMOUNT_RESISTANCE_DURATION_TICKS = 5 * 20; // 5 seconds - config: dragon_settings.dismount_resistance_duration_ticks
    private static final int DISMOUNT_RESISTANCE_AMPLIFIER = 254; // Max amplifier - config: dragon_settings.dismount_resistance_amplifier
    private static final long EMPTY_DESPAWN_DELAY_MILLIS = 5 * 60 * 1000; // 5 minutes - config: timing.despawn_delay_seconds
    private static final int DEATH_PARTICLE_COUNT = 20; // config: visual.death_particle_count
    private static final int PARTICLE_EFFECT_INTERVAL_TICKS = 5; // config: visual.particle_effect_interval_ticks

    // --- Flight Physics Constants ---
    // Default values - these can be overridden by config values when the plugin initializes
    private static final double ACCELERATION_RATE = 0.08; // How quickly the dragon gains speed - config: flight_physics.acceleration_rate
    private static final double DRAG_COEFFICIENT = 0.025; // How quickly the dragon slows down naturally - config: flight_physics.drag_coefficient
    private static final double GRAVITY_ACCEL = 0.04;     // Constant downward pull - config: flight_physics.gravity_accel
    private static final double DIVE_ACCEL_MULTIPLIER = 1.2; // Extra downward acceleration when diving steeply - config: flight_physics.dive_accel_multiplier
    private static final double LIFT_COEFFICIENT = 0.05; // Upward force generated by forward motion/pitch - config: flight_physics.lift_coefficient
    private static final float MAX_VERTICAL_SPEED_UP = 1.0f; // Maximum upward speed cap - config: flight_physics.max_vertical_speed_up
    private static final float MAX_VERTICAL_SPEED_DOWN = -2.0f; // Maximum downward speed cap (negative) - config: flight_physics.max_vertical_speed_down
    private static final float MIN_LIFT_SPEED = 0.2f; // Minimum forward speed required to generate significant lift - config: flight_physics.min_lift_speed
    private static final float FLAT_FLIGHT_SPEED_MULTIPLIER = 1.5f; // Speed multiplier for level flight - config: flight_physics.flat_flight_speed_multiplier
    ...
    private String dragonType = null; // Changed from "Fire Dragon" to null to avoid overriding NBT value
    ...
    public void initialize(DragonPlugin plugin) {
        ...
        if (plugin != null) {
            ...
            // Ensure dragon type is loaded immediately
            if (this.dragonType == null && this.namedTag != null && this.namedTag.contains(NBT_KEY_DRAGON_TYPE)) {
                this.dragonType = this.namedTag.getString(NBT_KEY_DRAGON_TYPE);
                plugin.getLogger().info("[DragonEntity Initialize] Loaded dragonType from NBT: '" + this.dragonType + "'");
            }
            
            // Initialize other parameters from configuration
            this.currentMaxSpeed = (float) plugin.getConfig().getDouble("dragon_stats.max_speed", MAX_SPEED);
        }
    }
    ...
    public EntityDefinition getEntityDefinition() {
        // Use a safe default if dragonType is null
    ...
    public void spawnTo(Player player) {
        // If dragonType is null, try to load it from NBT again
        if (this.dragonType == null && this.namedTag != null && this.namedTag.contains(NBT_KEY_DRAGON_TYPE)) {
            this.dragonType = this.namedTag.getString(NBT_KEY_DRAGON_TYPE);
            if (plugin != null) {
                plugin.getLogger().info("[DragonEntity SpawnTo] Recovered dragonType from NBT: '" + this.dragonType + "'");
            }
        }
    ...
    public boolean dismountEntity(Entity entity) {
        ...
        if (entity instanceof Player) {
            ...
            // Get resistance settings from config
            int resistanceDurationTicks = plugin.getConfig().getInt("dragon_settings.dismount_resistance_duration_ticks", DISMOUNT_RESISTANCE_DURATION_TICKS);
            int resistanceAmplifier = plugin.getConfig().getInt("dragon_settings.dismount_resistance_amplifier", DISMOUNT_RESISTANCE_AMPLIFIER);
            
            player.addEffect(cn.nukkit.potion.Effect.getEffect(cn.nukkit.potion.Effect.RESISTANCE)
                .setDuration(resistanceDurationTicks)
                .setAmplifier(resistanceAmplifier));
        }
    ...
    public boolean onUpdate(int currentTick) {
        ...
        if (!this.passengers.isEmpty()) {
            ...
            // Get flight physics constants from config
            double accelerationRate = plugin.getConfig().getDouble("flight_physics.acceleration_rate", ACCELERATION_RATE);
            double dragCoefficient = plugin.getConfig().getDouble("flight_physics.drag_coefficient", DRAG_COEFFICIENT);
            double gravityAccel = plugin.getConfig().getDouble("flight_physics.gravity_accel", GRAVITY_ACCEL);
            double diveAccelMultiplier = plugin.getConfig().getDouble("flight_physics.dive_accel_multiplier", DIVE_ACCEL_MULTIPLIER);
            double liftCoefficient = plugin.getConfig().getDouble("flight_physics.lift_coefficient", LIFT_COEFFICIENT);
            float maxVerticalSpeedUp = (float) plugin.getConfig().getDouble("flight_physics.max_vertical_speed_up", MAX_VERTICAL_SPEED_UP);
            float maxVerticalSpeedDown = (float) plugin.getConfig().getDouble("flight_physics.max_vertical_speed_down", MAX_VERTICAL_SPEED_DOWN);
            float minLiftSpeed = (float) plugin.getConfig().getDouble("flight_physics.min_lift_speed", MIN_LIFT_SPEED);
            float flatFlightSpeedMultiplier = (float) plugin.getConfig().getDouble("flight_physics.flat_flight_speed_multiplier", FLAT_FLIGHT_SPEED_MULTIPLIER);
            
            // 1. Adjust current speed towards target speed (Acceleration/Deceleration & Drag)
            if (this.currentFlightSpeed < this.targetSpeed) {
                this.currentFlightSpeed += accelerationRate * (this.targetSpeed - this.currentFlightSpeed);
                ...
                this.currentFlightSpeed -= accelerationRate * (this.currentFlightSpeed - this.targetSpeed) * 1.5; 
                this.currentFlightSpeed = Math.max(this.currentFlightSpeed, this.targetSpeed); // Clamp to target
            }
            // Apply Drag (Natural Deceleration)
            this.currentFlightSpeed -= this.currentFlightSpeed * dragCoefficient;
            ...
            this.motionY -= gravityAccel; // Apply gravity
            ...
            if (horizontalSpeed > minLiftSpeed && this.pitch < 0) { // Lift generation
                // Lift opposes gravity, scaled by horizontal speed and how much looking up (cos(pitch)) 
                double liftForce = horizontalSpeed * liftCoefficient * Math.cos(pitchRad); 
                this.motionY += liftForce;
            }
            
            // Get pitch thresholds from config
            float pitchThresholdForDive = (float) plugin.getConfig().getDouble("flight_physics.pitch_threshold_down", 20);
            
            if (this.pitch > pitchThresholdForDive) { // Dive bonus acceleration
                 double diveBonus = Math.sin(Math.toRadians(this.pitch - pitchThresholdForDive)) * gravityAccel * diveAccelMultiplier;
            ...
            this.motionY = Math.max(maxVerticalSpeedDown, Math.min(maxVerticalSpeedUp, this.motionY));
        ...
        if (this.passengers.isEmpty() && shouldDespawn) {
            ...
            // Get despawn delay from config (in seconds) and convert to milliseconds
            long despawnDelayMillis = (long) (plugin.getConfig().getDouble("timing.despawn_delay_seconds", 300) * 1000);
            if (elapsed >= despawnDelayMillis) {
                this.close();
                return false;
            }
        }
        ...
        if (super.onUpdate(currentTick)) {
            // Get particle effect interval from config
            int particleEffectInterval = plugin.getConfig().getInt("visual.particle_effect_interval_ticks", PARTICLE_EFFECT_INTERVAL_TICKS);
            if (currentTick % particleEffectInterval == 0 && !this.getViewers().isEmpty()) {
                applyCustomizations();
            }
            return true;
        }
    ...
    public void onPlayerInput(Player player, double strafe, double forward, float packetPitch, float packetYaw) {
        ...
        float baseMaxSpeed = (float) plugin.getConfig().getDouble("dragon_stats.max_speed", MAX_SPEED);
        this.currentMaxSpeed = baseMaxSpeed; // Reset to base max speed
        
        // Get flat flight threshold and multiplier from config
        float pitchThresholdForFlatFlight = (float) plugin.getConfig().getDouble("flight_physics.pitch_threshold_flat", 15);
        float flatFlightSpeedMultiplier = (float) plugin.getConfig().getDouble(
            "flight_physics.flat_flight_speed_multiplier", FLAT_FLIGHT_SPEED_MULTIPLIER);
        
        if (Math.abs(playerPitch) < pitchThresholdForFlatFlight) { // Check if pitch is relatively level
            this.currentMaxSpeed *= flatFlightSpeedMultiplier;
        }
    ...
    protected DataPacket createAddEntityPacket() {
        ...
        if (this.dragonType == null && this.namedTag != null && this.namedTag.contains(NBT_KEY_DRAGON_TYPE)) {
            this.dragonType = this.namedTag.getString(NBT_KEY_DRAGON_TYPE);
            if (plugin != null) {
                plugin.getLogger().info("[DragonEntity createAddEntityPacket] Recovered dragonType from NBT: '" + this.dragonType + "'");
            }
        }
            ...
            case "Ice Dragon":
                if (source.getCause() == EntityDamageEvent.DamageCause.DROWNING) {
                    isImmune = true;
                           ...
                           source.getCause() == EntityDamageEvent.DamageCause.FIRE_TICK || 
                           source.getCause() == EntityDamageEvent.DamageCause.LAVA) {
                    // Use config value for ice dragon fire vulnerability
    ...
    private void handleDeath() {
        ...
        if (this.level != null) {
            // Get death particle count from config
            int deathParticleCount = plugin.getConfig().getInt("visual.death_particle_count", DEATH_PARTICLE_COUNT);
    ...
    private void shootWaterball() {
        ...
        // Create and spawn waterball - use new EntityWaterBall class
        CompoundTag nbt = Entity.getDefaultNBT(spawnPos);
        nbt.putLong("DragonID", this.getId());
        EntityWaterBall waterball = new EntityWaterBall(plugin, this.getChunk(), nbt, this);
    ...
    private void shootEarthball() {
        ...
        // Create and spawn earthball - use new EntityEarthBall class
        CompoundTag nbt = Entity.getDefaultNBT(spawnPos);
        nbt.putLong("DragonID", this.getId());
        EntityEarthBall earthball = new EntityEarthBall(plugin, this.getChunk(), nbt, this);
    ...
    /**
     * Sets the current plugin-managed health of the dragon.
     * @param health The health value to set.
     */
    public void setPluginHealth(float health) {
        this.pluginHealth = Math.min(health, this.pluginMaxHealth);
    }
